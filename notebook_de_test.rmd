...
# diffman, que les carreaux internes aux zones,il fautéviter les carreaux intersectant, les blanchir ne servirait pas à protéger les autres zones -> préférer les carreaux internes
fully_included_z2 <- isolate_intersection(input_dt)$fully_included_z2
list_z2<-unique(fully_included_z2$z2)
z2_to_tag <- data.table(z2  = list_z2, tag  = FALSE)

comp_diff_info <- global_diff_info[id_comp == num_comp]

l <- split(comp_diff_info,comp_diff_info$checked_area)
...
for(area_issue in l){
  ...
  z2_full_incl <- merge(fully_included_z2[z1 %in% z1_in_area],z2_to_tag,by ="z2",nomatch = 0L)[order(nb_obs)] # la zone a risque
  z2_full_excl <- merge(fully_included_z2[!z1 %in% z1_in_area],z2_to_tag,by ="z2",nomatch = 0L)[order(nb_obs)] # le complementazire dans la composante
...
# pour une intersetion à probleme garder une seule ligne. ET blanchir un carreau dans la zone checked et dans le complémentaire de la zone checked. dans la composante -> en images :
...
## TO DO :
- mettre la population communale dans les communes,  coloration des carreaux en blanchis
