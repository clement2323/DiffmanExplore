...
Donc je vais reconstruire ces zone si elles n 'existent pas d'où la fonction

```{r}
# ok pour toutes les external je regarde si l'internal est là
...
protect_component <- function(id_comp,global_diff_info){ 
  #id_comp <- 136
  list_z1_compo<- compo$z1[compo$id_comp == id_comp]
  ...
  for(area_issue in l){
    ...
    # On ne se limite à rien pour la diff externe
    z2_full_excl <- merge(input_dt[!z1 %in% z1_in_area],z2_to_tag,by ="z2",nomatch = 0L)[order(nb_obs)] # la zone a risque
...
#comp_to_nb_com
z2_to_tag <- protect_component(136,global_diff_info)
list_z1 <- compo[id_comp == 136]$z1
